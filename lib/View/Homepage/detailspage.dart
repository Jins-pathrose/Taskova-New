...
import 'package:flutter/material.dart'; // Still needed for some widgets like FadeTransition
...
class _JobDetailPageState extends State<JobDetailPage>
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          if (_isAccepted) ...[SizedBox(height: 16), _buildReviewSection()],
          _buildBottomActions(),
        ],
      ),
    );
  ...
  Future<void> _applyForJob() async {
    ...
    try {
      ...
      if (accessToken == null) {
        _showErrorMessage(context, 'Please log in to apply for jobs.');
        return;
      }
      ...
      if (response.statusCode == 201) {
        ...
        _showSuccessMessage(context, 'Application submitted successfully!');
        _checkIfJobIsAccepted();
      } else if (response.statusCode == 400) {
        final errorData = jsonDecode(response.body);
        _showErrorMessage(
          context,
          errorData['message'] ?? 'Application failed.',
        );
      } else {
        _showErrorMessage(
          context,
          'You have already applied for this job. Please wait while your application is being reviewed.',
        );
      }
    } catch (e) {
      _showErrorMessage(
