    ...
    try {
     ...
     final response = await http.get(
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint('Failed to fetch job requests. Code: ${response.statusCode}');
        setState(() => isLoading = false);
      }
    } catch (e) {
                ...
                ? Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 8),
                        Text(
                          appLanguage.get('Check_back_later_for_new_opportunities'),
                          style: TextStyle(
                            fontSize: 14,
...
onTap: () {
  Navigator.push(
    context,
    CupertinoPageRoute(
      builder: (_) => JobRequestDetailPage(
        data: req,
        jobRequestId: req['id'], // 5 in your example
        chatRoomId: req['chat_room_id'], // 8 in your example
        requesterId: requester['id'], // 10 in your example
        driverId: req['driver']['id'], // 27 in your example
      ),
    ),
  );
},
